...
/**
 * SVG & ICO files upload
 */

function mfn_check_filetype_and_ext_svg( $types, $file, $filename, $mimes ) {

  $theme_disable = mfn_opts_get('theme-disable');
  if ( empty($theme_disable['svg-allow']) && current_user_can('edit_theme_options') && false !== strpos( $filename, '.svg' ) ) {
    $types['ext'] = 'svg';
...
add_filter( 'wp_check_filetype_and_ext', 'mfn_check_filetype_and_ext_svg', 10, 4 );
...
if (! function_exists('mfn_breadcrumbs')) {
	function mfn_breadcrumbs($params = false)
	{
		...
		if( empty($params['include_home']) ) $breadcrumbs[] = '<a href="'. esc_attr(home_url()) .'">'. esc_html($translate['home']) .'</a>';
